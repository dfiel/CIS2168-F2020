...
public class MaxPQ<Key> {
   ...
   private void swim(int k) {
      // YOUR CODE HERE
      while (k > 1 && less(k/2,  k)) { // if we are not at the top of the tree, and the parent is less than k
   ...
   private void sink(int k) {
      ...
      while(2*k <= n){ // while there are more children
         ...
         if (j < n && less(j, j+1)) j++; // if left child less than right child, check right child instead
         if (!less(k, j)) break; // if element is greater than either child, we are done
