...
public class MinPQ<Key> implements Iterable<Key> {
   ...
   private void swim(int k) {
      // YOUR CODE HERE
      while (k > 1  && greater(k/2, k)){ // if we are not at the top of the tree, and the parent is greater than k
         exch(k, k/2); // swap k and parent
         k = k/2; // check next parent
      }
   }

   private void sink(int k) {
      // YOUR CODE HERE
      while (2*k <= n){ // while there are more children
         int j = 2*k; // check left child
         if (j < n && greater(j, j+1)) j++; // if left child is larger than right child, check right child instead
         if (!greater(k, j)) break; // if element is smaller than either child, we are done
         exch(k, j); // otherwise swap k and child
         k = j; // check next children
      }
   }
